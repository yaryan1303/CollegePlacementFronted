...
const Students = () => {
  ...
  const [filteredStudents, setFilteredStudents] = useState([]); // Fixed typo here
  ...
  useEffect(() => {
    // Apply filtering whenever students or filters change
    applyFilters();
  ...
  const applyFilters = () => {
    ...
    setFilteredStudents(filtered); // Fixed typo here
  };
  ...
  const currentItems = filteredStudents.slice(indexOfFirstItem, indexOfLastItem); // Fixed typo here
  const totalPages = Math.ceil(filteredStudents.length / itemsPerPage); // Fixed typo here
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          <div>
            <h2 className="text-lg font-medium text-gray-900">
              Students <span className="text-gray-500">({filteredStudents.length})</span> {/* Fixed typo here */}
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, filteredStudents.length)} of {filteredStudents.length} entries {/* Fixed typo here */}
            </p>
          </div>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{Math.min(currentPage * itemsPerPage, filteredStudents.length)}</span> of{' '} {/* Fixed typo here */}
                <span className="font-medium">{filteredStudents.length}</span> results {/* Fixed typo here */}
              </p>
            </div>
            <div>
        ...
        {filteredStudents.length === 0 && !loading && ( // Fixed typo here
